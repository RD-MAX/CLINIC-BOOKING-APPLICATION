        ...
        try {
            ...
            if ("checkout.session.completed".equals(event.getType())) {
                ...
                if (data.getObject().isPresent()) {
                    session = (Session) data.getObject().get();
                    System.out.println("✅ Session deserialized normally");
                }
                // ✅ FALLBACK (IMPORTANT FIX)
                else {
                    System.out.println("⚠️ Using fallback deserialization");

                    String json = data.getRawJson();
